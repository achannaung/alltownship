...
export default function VillageDetailPanel({
  ...
  // Dummy Map SVG generation for visual feedback
  const mapGridPattern = () => {
    const lines = [];
    for (let i = 20; i < 300; i += 40) {
      lines.push(<line key={`h-${i}`} x1="0" y1={i} x2="300" y2={i} stroke="rgba(255,255,255,0.06)" strokeWidth="1" />);
      lines.push(<line key={`v-${i}`} x1={i} y1="0" x2={i} y2="300" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />);
    }
    return lines;
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-slate-900/40 rounded-xl p-4 border border-slate-800">
            <div className="flex items-center justify-between mb-3">
              ...
              {flaggedState?.flaggedAt && (
                <span className="text-[10px] text-slate-500 flex items-center gap-1">
                  <Calendar size={10} />
                  Updated {new Date(flaggedState.flaggedAt).toLocaleDateString()}
                </span>
              )}
            </div>
            <div className="grid grid-cols-4 gap-2">
              {(['pending', 'monitored', 'alert', 'inactive'] as const).map((status) => {
                const isActive = (flaggedState?.status || 'pending') === status;
                const statusColors = {
                  pending: isActive ? 'bg-amber-500/20 text-amber-300 border-amber-500' : 'hover:bg-amber-500/5 text-slate-400 hover:text-amber-300 border-slate-800',
                  monitored: isActive ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500' : 'hover:bg-emerald-500/5 text-slate-400 hover:text-emerald-300 border-slate-800',
                  alert: isActive ? 'bg-rose-500/20 text-rose-300 border-rose-500 animate-pulse' : 'hover:bg-rose-500/5 text-slate-400 hover:text-rose-300 border-slate-800',
                  inactive: isActive ? 'bg-slate-500/20 text-slate-300 border-slate-500' : 'hover:bg-slate-500/5 text-slate-400 hover:text-slate-300 border-slate-800',
                };
                ...
                return (
                  <button
                    key={status}
                    type="button"
                    onClick={() => onUpdateStatus(village.id, status)}
                    className={`border rounded-lg py-2 text-xs font-semibold capitalize transition cursor-pointer text-center ${statusColors[status]}`}
                  >
                    {status}
                  </button>
                );
          ...
          {/* Geographic Metadata & Map Mockup */}
          <div className="bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden">
            ...
            {/* Dummy Map SVG container */}
            <div className="relative h-44 bg-slate-950/80 overflow-hidden flex items-center justify-center">
              <svg className="absolute inset-0 w-full h-full">
                {mapGridPattern()}
                {/* Styled geographic visual contours */}
                <path d="M 40,80 Q 90,30 180,90 T 260,110" fill="none" stroke="rgba(255,255,255,0.02)" strokeWidth="4" />
                <path d="M 10,130 Q 110,180 150,110 T 290,140" fill="none" stroke="rgba(255,255,255,0.02)" strokeWidth="3" />
                {/* Coordinates grid markings */}
                <text x="10" y="20" fill="rgba(255,255,255,0.2)" fontSize="9" fontFamily="monospace">LAT: {village.latitude.toFixed(4)}</text>
                <text x="10" y="35" fill="rgba(255,255,255,0.2)" fontSize="9" fontFamily="monospace">LON: {village.longitude.toFixed(4)}</text>
              </svg>

              {/* Pulsing indicator pin */}
              <div className="absolute flex flex-col items-center">
                <div className="absolute w-8 h-8 rounded-full bg-indigo-500/30 animate-ping"></div>
                <div className="p-2 bg-indigo-600 rounded-full border border-indigo-300 shadow-xl relative z-10 text-white">
                  <MapPin size={16} />
                </div>
                <span className="bg-slate-900/90 text-white border border-slate-800 text-[10px] px-2 py-0.5 rounded shadow-lg mt-1 whitespace-nowrap">
                  {village.nameEn}
                </span>
              </div>
            </div>
