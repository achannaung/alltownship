...
import { Database, Search, Sparkles, ShieldAlert, CheckCircle2, ChevronRight, SlidersHorizontal, Map, Loader2, Wifi, WifiOff, AlertTriangle, Palette, Moon } from 'lucide-react';
import { fetchAndParseNationalRegistry } from './utils/csvParser';

export default function App() {
  // --- Theme State ---
  const [theme, setTheme] = useState<'vibrant' | 'neutral'>(() => {
    try {
      return (localStorage.getItem('mimu_theme') as 'vibrant' | 'neutral') || 'vibrant';
    } catch {
      return 'vibrant';
    }
  });
  ...
  useEffect(() => {
    if (theme === 'neutral') {
      ...
      document.body.classList.remove('theme-neutral');
    }
    try {
    ...
    <div className="min-h-screen py-10 px-4 md:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex justify-center sm:justify-end items-center gap-2">
          <div className="inline-flex items-center gap-1 p-1 bg-slate-950/80 backdrop-blur-md rounded-full border border-slate-800 shadow-2xl">
            <button
              id="theme-btn-vibrant"
              onClick={() => setTheme('vibrant')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide transition cursor-pointer ${
                theme === 'vibrant'
                  ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30'
                  : 'text-slate-400 hover:text-white hover:bg-slate-900'
              }`}
            >
              <Palette size={13} className={theme === 'vibrant' ? 'animate-pulse' : ''} />
              <span>Vibrant Gradient</span>
            </button>
            <button
              id="theme-btn-neutral"
              onClick={() => setTheme('neutral')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide transition cursor-pointer ${
                theme === 'neutral'
                  ? 'bg-slate-800 text-blue-400 border border-slate-700/80 shadow-lg'
                  : 'text-slate-400 hover:text-white hover:bg-slate-900'
              }`}
            >
              <Moon size={13} />
